...
function collectStatHourly(beginTime, endTime, callbackStat) {
    function postProcess(perfViewResult, respRangeResult) {
        ...
        for(var i in perfViewResult) {
            ...
            for(;j < respRangeResult.length; j++) {
                var summaryBeginTime = new Date(summary.beginTime);
                var respBeginTime = new Date(respRangeResult[j].beginTime);

                if(summaryBeginTime.getTime() == respBeginTime.getTime()) {
                    summary.rangeOfResponseTime.push(respRangeResult[j]);
                    push = true;
                } else if(summaryBeginTime.getTime() < respBeginTime.getTime()) {
                    break;
                } else {
                    continue;
                }
    ...
    function summarySelectQuery() {
        var queryStr =
            'select ' +
            "date_format(update_time, '%Y-%m-%d %H:00:00') beginTime, date_format(update_time + interval 1 hour, '%Y-%m-%d %H:00:00')  endTime, " +
            ...
            "order by date_format(update_time, '%Y-%m-%d %H:00:00') ";
    ...
    function respRangeSelectQuery() {
        var queryStr =
            ...
            'where update_time between ? and ? ' +
            "group by date_format(update_time, '%Y-%m-%d %H:00:00'), resp_range " +
            "order by date_format(update_time, '%Y-%m-%d %H:00:00'), resp_range ";
        return queryStr;
    }
